...
// Küresel Durum Bayrağı - Kayıt tetiklemek için kritik
...
int yukseklikAl(IntervalNode *n) { 
...
void salonUlasimDijkstra(int grafik[MAX_SALON][MAX_SALON], int baslangic) {
    ...
    for (int s = 0; s < MAX_SALON - 1; s++) {
        ...
        ziyaret[u] = 1;
        for (int v = 0; v < MAX_SALON; v++)
            if (!ziyaret[v] && grafik[u][v] && mesafe[u] != INF && mesafe[u] + grafik[u][v] < mesafe[v])
                mesafe[v] = mesafe[u] + grafik[u][v];
    }
    printf("\n--- Salon 0'dan Diger Salonlara En Kisa Ulasim (Dijkstra) ---\n");
    for (int i = 0; i < MAX_SALON; i++) {
        printf("Salon %d: %d dk\n", i, mesafe[i]);
    }
}
...
IntervalNode* dugumOlustur(Randevu r) {
    ...
    *(yeni->veri) = r;
    ...
    yeni->yukseklik = 1;
    sonIslemBasarili = 1; // Başarı bayrağını tetikle
    return yeni;
}
...
IntervalNode* randevuEkle(IntervalNode* kok, Randevu r, int sessizMod) {
    ...
    if (cakismaVarMi(r, *(kok->veri))) {
        if (!sessizMod) printf("\aHATA: ID %d ile cakisma! Bekleme listesine alindi.\n", kok->veri->id);
        sonIslemBasarili = 0;
        return kok;
    }
...
// --- 7. SIRALAMA VE ARAMA (Quicksort & Binary Search) ---
...
void quicksort(Randevu dizi[], int d, int y) {
    if (d < y) {
        ...
        Randevu temp = dizi[i+1]; dizi[i+1] = dizi[y]; dizi[y] = temp;
        int pi = i + 1;
        quicksort(dizi, d, pi - 1);
        quicksort(dizi, pi + 1, y);
    }
}
...
int main() {
    ...
    do {
        printf("\n=== IMU SALON YONETIM MOTORU ===\n");
        printf("1. Yeni Randevu Ekle (AVL & Interval Tree)\n");
        printf("2. Bekleme Listesini Gor (Queue)\n");
        printf("3. Salonlar Arasi Mesafe (Dijkstra)\n");
        printf("4. Sirali Listele ve Ara (Sort & Search)\n");
        printf("5. Cikis\n");
        printf("Seciminiz: ");
        scanf("%d", &secim);
        ...
        if (secim == 1) {
            ...
            printf("Isim: "); scanf(" %[^\n]s", y.isim);
            printf("Bas (SS:DD): "); scanf("%d:%d", &s1, &d1);
            printf("Bit (SS:DD): "); scanf("%d:%d", &s2, &d2);
            ...
            if (sonIslemBasarili) {
                ...
                push(&undoStack, y.id);
                jsonDosyasiOlustur(kok);
                printf("Basariyla eklendi ve Web guncellendi!\n");
            } else {
                enqueue(&bekleme, y);
            }
        } 
            ...
            if (t == NULL) printf("Liste bos.\n");
            while(t) {
                printf("ID:%d | Isim:%s | Saat:%02d:%02d\n", 
                       t->veri.id, t->veri.isim, t->veri.baslangic/60, t->veri.baslangic%60);
                t = t->sonraki;
            }
        } 
        else if (secim == 3) {
            salonUlasimDijkstra(grafik, 0);
        }
        else if (secim == 4) {
            Randevu liste[100];
